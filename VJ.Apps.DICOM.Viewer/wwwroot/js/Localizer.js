...
Localizer.drawScout = function() {
	...
	for ( var i = 0; i < frames.length; i++) {
		...
		let serUid = contents.find('#serId').text().split('_')[0];
		let refSerUid = jQuery('#serId').text().split('_')[0];
		
		let activeBorder = 'rgb(255, 138, 0)';
		// scout line should not be shown if same series is opened more than once
		if (contents.find('body').css('border-color') !== activeBorder && serUid.indexOf(refSerUid) < 0) {
		
			var modality = contents.find('#modalityDiv').text().indexOf('CT') >= 0 ? 'CT' : 'MR';
			var referenceImgType = jQuery(frames[i]).contents().find('#imgType').text().trim();
			if (modality === 'CT') {
				if (referenceImgType === 'LOCALIZER') {
				...
				} else if (referenceImgType === 'AXIAL' || referenceImgType === "MPR") {
					project(contents, modality);
				} else {
